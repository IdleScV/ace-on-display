 ...
 *
 * If E2E_EMAIL/E2E_PASSWORD aren't supplied, call the sandbox
 * `/api/public/e2e/bootstrap` endpoint to provision a throwaway superadmin
 * and inject the credentials into process.env so worker processes inherit
 * them. The endpoint is hostname-gated (only Lovable preview/dev hosts and
 * localhost), so no secret is required.
 */
  ...
  if (!res.ok) {
    throw new Error(
      `[e2e:global-setup] Bootstrap failed (${res.status}): ${await res.text()}\n` +
        `Endpoint is only available on preview/dev hosts — check baseURL: ${baseURL}`,
    );
  }
